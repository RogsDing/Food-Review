Page({
  data: {
    ...
    selectedCompare: '',
    compareOptions: [
      { value: 'better', icon: '📈', label: '比上次好' },
      { value: 'same', icon: '➡️', label: '差不多' },
      { value: 'worse', icon: '📉', label: '不如上次' }
    ]
  },
    ...
    wx.getSetting({
      success: function (settingRes) {
        ...
        if (locationAuth === false) {
          wx.showModal({
            title: '位置授权',
            content: '需要获取您的位置信息来快速定位所在城市，请在设置中开启定位权限',
            confirmText: '去设置',
  ...
  doGetLocation: function () {
    ...
    wx.getLocation({
      ...
      success: function (res) {
        ...
        console.log('定位成功，坐标:', latitude, longitude);

        wx.request({
          url: `https://apis.map.qq.com/ws/geocoder/v1/?location=${latitude},${longitude}&key=OB4BZ-D4W3U-B7VVO-4PJWW-6TKDJ-WPB77`,
          success: function (res) {
            console.log('逆地理编码API返回:', res.data);
            if (res.data.status === 0) {
              const addressComponent = res.data.result.address_component;
              const city = addressComponent.city.replace('市', '');
              const district = addressComponent.district;
              console.log('解析城市:', city, '区:', district);

              const cities = that.data.regionOptions[0];
              let matchedProvince = null;
              let matchedCityList = null;
              let provinceIndex = -1;

              const directIndex = cities.indexOf(city);
              if (directIndex >= 0) {
                matchedProvince = cities[directIndex];
                matchedCityList = that.data.provinceCityMap[cities[directIndex]];
                provinceIndex = directIndex;
              } else {
                for (let i = 0; i < cities.length; i++) {
                  const prov = cities[i];
                  const cityList = that.data.provinceCityMap[prov];
                  if (cityList && cityList.length > 0) {
                    for (let j = 0; j < cityList.length; j++) {
                      const c = cityList[j].replace('市', '');
                      if (c === city || cityList[j].indexOf(city) >= 0) {
                        matchedProvince = prov;
                        matchedCityList = cityList;
                        provinceIndex = i;
                        break;
                      }
                    }
              ...
              if (matchedProvince) {
                const cityListIndex = matchedCityList.indexOf(city) >= 0 ? matchedCityList.indexOf(city) : matchedCityList.findIndex(function(c) { return c.indexOf(city) >= 0; });
                ...
                that.setData({
                  ...
                  city: city,
                  area: city,
                  regionIndex: [provinceIndex, cityListIndex >= 0 ? cityListIndex : 0]
                });
                ...
                wx.showToast({
                  title: '定位成功，但城市"' + city + '"不在选项中',
                  icon: 'none'
                });
              }
            } else {
              console.error('逆地理编码失败，status:', res.data.status, 'message:', res.data.message);
              wx.showToast({
                title: '定位失败：' + (res.data.message || '未知错误'),
                icon: 'none'
              });
            }
          },
          fail: function (err) {
            console.error('逆地理编码API请求失败:', err);
            wx.showToast({
              title: '定位失败，请检查网络',
              icon: 'none'
            });
      ...
      fail: function (err) {
        ...
        wx.showModal({
          title: '需要定位权限',
          content: '请授权定位权限以快速定位您所在的城市，点击"去设置"开启',
          confirmText: '去设置',
          success: function (modalRes) {
            if (modalRes.confirm) {
              wx.openSetting();
  ...
  handleSliderChange(e) {
    ...
    this.setData({
      rating: roundedValue,
      ratingFloor: Math.floor(roundedValue)
    });
  },
  ...
  onShopNameInput: function (e) {
    ...
    if (name.length >= 2) {
      this.checkShopHistory(name);
    }
  },

  checkShopHistory: function (shopName) {
    wx.cloud.callFunction({
      ...
      if (res.result && res.result.success && res.result.exists) {
        ...
        this.setData({ showShopHistory: false });
      }
    }).catch(err => {
      console.log('检查店铺历史失败:', err);
    });
  },

  selectCompareOption: function (e) {
    var value = e.currentTarget.dataset.value;
    var ratingAdjustment = { better: 0.5, same: 0, worse: -0.3 };
    var currentRating = this.data.rating + ratingAdjustment[value];
    ...
    wx.showToast({ title: '评分已调整', icon: 'none' });
  },

  closeShopHistory: function () {
    this.setData({ showShopHistory: false });
  },

  confirmShopHistory: function () {
    this.setData({ showShopHistory: false });
  },
